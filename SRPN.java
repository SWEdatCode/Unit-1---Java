...
public class SRPN {
  ...
  // If the stack isn't full (above 23 numbers),
  // add the number passed to the
  // stack, otherwise, output stack overflow
  ...
  // addition function
  public void addition(int lastVal) { // Last value parameter used,
    // to add value back to stack in the event of error
    try {
      int num1 = nums.pop();
      int num2 = nums.pop();
      ...
      System.out.println("Stack underflow."); // If there are not enough operands to perform calc then,
      // display error
  ...
  public void divide(int lastVal) {
    try {
      ...
      } else { // If top value is 0 then remove the top two using pop,
        // push a zero onto the
    ...
    } catch (EmptyStackException e) { // If method can not be performed display stack underflow
  ...
  public void display() {
    ...
    if (stackSize == 0) {
      ...
      for (int j = 0; j < stackSize; j++) { // Push top value from stack
        // into temporary display stack and pop from
        // main nums stack
